...
class LokiUI:
    def __init__(self, face_path, size=None):
        self.root = tk.Tk()
        self.root.title("L.O.K.I — REY XXXVII")
    ...
    def _draw(self):
        ...
        c.create_text(W // 2, 44, text="Logical Operational Kinetic Intelligence",
        ...
        if self.muted:
            ...
            stat = "● SPEAKING"
            sc   = C_ACC
        elif self._loki_state == "THINKING":
            sym  = "◈" if self.status_blink else "◇"
            stat = f"{sym} THINKING"
            sc   = C_ACC2
        elif self._loki_state == "PROCESSING":
            sym  = "▷" if self.status_blink else "▶"
            stat = f"{sym} PROCESSING"
            sc   = C_ACC2
        elif self._loki_state == "LISTENING":
            sym  = "●" if self.status_blink else "○"
        ...
        c.create_text(W // 2, H - 14, fill=C_DIM, font=("Courier", 8),
                      text="FatihMakes Industries  ·  CLASSIFIED  ·  REY XXXVII")
    ...
    def write_log(self, text: str):
        ...
        elif tl.startswith("loki:") or tl.startswith("ai:"):
    ...
    def _show_setup_ui(self):
        ...
        tk.Label(
            self.setup_frame,
            text="Configure L.O.K.I. before first boot.",
            fg=C_MID, bg="#00080d",
            font=("Courier", 9)
        ).pack(pady=(0, 14))
    ...
    def _save_api_keys(self):
        ...
        self.set_state("LISTENING")
        self.write_log(f"SYS: Systems initialised. OS -> {os_system.upper()}. LOKI online.")

    def _show_auth_ui(self):
        self.auth_frame = tk.Frame(
            self.root, bg=C_LOKI_BG,
            highlightbackground=C_LOKI_GOLD, highlightthickness=2
        )
        self.auth_frame.place(relx=0.5, rely=0.5, anchor="center", width=400, height=450)

        # Header with Loki aesthetic
        tk.Label(
            self.auth_frame,
            text="L.O.K.I  SYSTEMS",
            fg=C_LOKI_GOLD, bg=C_LOKI_BG,
            font=("Courier", 18, "bold")
        ).pack(pady=(30, 5))

        tk.Label(
            self.auth_frame,
            text="LOGICAL OPERATIONAL KINETIC INTELLIGENCE",
            fg=C_LOKI_GREEN, bg=C_LOKI_BG,
            font=("Courier", 8)
        ).pack(pady=(0, 25))

        # Username / Email
        tk.Label(
            self.auth_frame, text="GMAIL / USERNAME",
            fg=C_LOKI_GOLD, bg=C_LOKI_BG, font=("Courier", 10)
        ).pack(anchor="w", padx=40)

        self.user_entry = tk.Entry(
            self.auth_frame, fg=C_LOKI_GREEN, bg=C_LOKI_DIM,
            insertbackground=C_LOKI_GREEN, borderwidth=0, font=("Courier", 12),
            highlightthickness=1, highlightbackground=C_LOKI_GREEN
        )
        self.user_entry.pack(fill="x", padx=40, pady=(5, 15))

        # Password
        tk.Label(
            self.auth_frame, text="PASSWORD",
            fg=C_LOKI_GOLD, bg=C_LOKI_BG, font=("Courier", 10)
        ).pack(anchor="w", padx=40)
        ...
        self.pass_entry = tk.Entry(
            self.auth_frame, fg=C_LOKI_GREEN, bg=C_LOKI_DIM,
            insertbackground=C_LOKI_GREEN, borderwidth=0, font=("Courier", 12),
            show="*", highlightthickness=1, highlightbackground=C_LOKI_GREEN
        )
        self.pass_entry.pack(fill="x", padx=40, pady=(5, 10))

        # Error Message Label
        self.auth_error_label = tk.Label(
            self.auth_frame, text="", fg=C_RED, bg=C_LOKI_BG, font=("Courier", 9)
        )
        self.auth_error_label.pack()

        # Buttons
        btn_frame = tk.Frame(self.auth_frame, bg=C_LOKI_BG)
        btn_frame.pack(pady=20)

        tk.Button(
            btn_frame, text="LOGIN", command=self._handle_login,
            bg=C_LOKI_GREEN, fg=C_LOKI_BG, font=("Courier", 10, "bold"),
            width=12, borderwidth=0, cursor="hand2", activebackground=C_LOKI_GOLD
        ).pack(side="left", padx=10)

        tk.Button(
            btn_frame, text="REGISTER", command=self._handle_register,
            bg=C_LOKI_BG, fg=C_LOKI_GREEN, font=("Courier", 10, "bold"),
            width=12, borderwidth=1, highlightbackground=C_LOKI_GREEN,
            cursor="hand2", activebackground=C_LOKI_DIM
        ).pack(side="left", padx=10)
    ...
    def _handle_login(self):
        ...
        if not user or not pwd:
            self.auth_error_label.config(text="Fields cannot be empty")
            return
        ...
        if success:
            ...
            self.auth_error_label.config(text=msg)
    ...
    def _handle_register(self):
        ...
        if success:
            self.auth_error_label.config(text="User Registered! Please Login.", fg=C_LOKI_GREEN)
        else:
            self.auth_error_label.config(text=msg, fg=C_RED)

    def _on_auth_success(self, username):
        self._authenticated = True
        self.auth_frame.destroy()
        self.write_log(f"SYS: User '{username}' authenticated.")
