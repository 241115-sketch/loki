"""
browser_control.py  —  LOKI Browser Action Module
====================================================
...
def _real_profile_dir(browser: str) -> str:
    ...
    for p in candidates:
        if p.exists():
            print(f"[Browser] SUCCESS: Real profile found for {browser}: {p}")
            return str(p)

    fallback = home / ".loki_profiles" / browser
    fallback.mkdir(parents=True, exist_ok=True)
    print(f"[Browser] WARNING: Real profile not found for {browser}, using: {fallback}")
    return str(fallback)
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".loki_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using LOKI profile")
                loki_profile_path = str(Path.home() / ".loki_profiles" / "firefox_loki")
                Path(loki_profile_path).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(loki_profile_path, **kwargs)

            await asyncio.sleep(0.5)  # let the browser settle
            # Always open a dedicated LOKI tab instead of grabbing pages[0],
            # which may already be about:blank or a stale restored tab.
            self._page = await self._context.new_page()
            print(f"[Browser] SUCCESS: Firefox launched")
            return

        # ── Safari (webkit) ───────────────────────────────────────────────────
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".loki_profiles" / "safari")
            ...
            self._page = await self._context.new_page()
            print(f"[Browser] SUCCESS: Safari launched")
            return
        ...
        try:
            ...
            self._page = await self._context.new_page()
            print(f"[Browser] SUCCESS: Launched [{label}] profile={profile}")
            return
        except Exception as e:
            print(f"[Browser] WARNING: Real profile failed for {label}: {e}")

        # 2) LOKI fallback profili
        loki_profile = str(Path.home() / ".loki_profiles" / self.browser_name)
        Path(loki_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with LOKI profile: {loki_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(loki_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] SUCCESS: Launched [{label}] with LOKI profile")
    ...
    async def go_to(self, url: str) -> str:
        ...
        • On PlaywrightTimeout we check whether the URL actually changed;
          if it did, treat it as success so LOKI won't retry endlessly.
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "loki_screenshot.png")
