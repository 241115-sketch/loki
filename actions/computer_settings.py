...
def brightness_set(value: int):
    ...
    if _OS == "Darwin":
        subprocess.run(["osascript", "-e",
            f'tell application "System Events" to set brightness of container 1 to {value/100.0}'],
            capture_output=True)
    elif _OS == "Linux":
...
def toggle_bluetooth():
    if _OS == "Windows":
        try:
            # Using PowerShell to toggle Bluetooth via Windows Runtime API
            ps_script = (
                ...
                "if ($bt) { if ($bt.State -eq 'On') { $bt.SetStateAsync('Off') } else { $bt.SetStateAsync('On') } }"
            )
        ...
        try:
            ...
            new_state = "0" if "1" in result.stdout else "1"
            subprocess.run(["blueutil", "-p", new_state])
        except Exception:
            pass
...
- If the user says "turn on X" or "turn off X", map to the corresponding toggle or set action.
